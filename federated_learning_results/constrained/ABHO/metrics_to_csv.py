...
def extract_metrics(base_path='.'):
    ...
    pattern_multiple = r'(\d+)-rounds/(\d+)-clients/fl_run_metrics_num_clients-\d+.csv'
    pattern = r'(\d+)-rounds/2-clients/fl_run_metrics_num_clients-2.csv'
    csv_files = []
    ...
    for csv_path in csv_files:
        try:
            ...
            match_multiple_clients = re.search(r'(\d+)-rounds/(\d+)-clients', normalized_path)
            match = re.search(r'(\d+)-rounds/2-clients', normalized_path)
            if match:
